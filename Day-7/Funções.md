# Função
É um bloco de código reutilivazel ou declarações de programa projetadas para solucionar um determinada atividade. Um função é declarada pela a palavra chave `function` seguido do seu nome e seguido por parênteses `()`, no qual pode haver parâmetros ou não. 

Para armazenar dados uma função tem que retorna um determinado tipo de dados.


Uma função pode ser declarada ou criada de varias formas:
